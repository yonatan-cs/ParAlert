...
## 🤖 מודלים אמיתיים + טאב "נסו בעצמכם" למצגת (ML service + ngrok)
...
4. `ngrok http 8100` → מעתיקים את כתובת ה-https.
5. ב-Render → Environment: `USE_MODEL=true` + `ML_SERVICE_URL=https://<id>.ngrok.app` → Save (redeploy ~30ש').
...
**חסין כשל:** אם ה-ML service/ngrok נופל → ה-backend חוזר אוטומטית ל-keyword הדו-לשוני (עברית+אנגלית)
ולא קורס. אחרי המצגת: מוחקים `ML_SERVICE_URL` ב-Render → חזרה למצב keyword היציב.
קישור ישיר לשופט: `…/?lang=en&tab=tryit`.

> חלופה פשוטה יותר (בלי שירות נפרד): להריץ את כל ה-backend מקומית עם `USE_MODEL=true` + ngrok ולהצביע
