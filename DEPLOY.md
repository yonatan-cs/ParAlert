...
**מוכן (בראנץ' `fs-dashboard`):**
- כתובת backend env-driven (`VITE_API_BASE`), WS נגזר (`http→ws`, `https→wss`).
- `vercel.json` (build של `frontend_dashboard/app`, SPA rewrites).
- fallback ל-mock כשאין backend → הדמו אף פעם לא ריק.

**מוכן ואומת:** `engines.node=20.x`, build נוקה והורץ כמו ש-Vercel מריץ (`vercel.json` → `dist/` עם index.html + assets + mock_data.json). פריסה בלי URL של FS-A תעבוד מול mock.

**צעדי פריסה (FS-B), לחיצה-לחיצה:**
1. vercel.com → Add New → Project → Import את הריפו `Hackathon-Project`.
2. Branch לפריסה: `fullstack` (האפליקציה המשולבת) או `main` אחרי מיזוג.
3. Framework Preset: **Other** (ה-`vercel.json` בשורש מגדיר build + output — לא לשנות Root Directory).
4. Environment Variables → להוסיף `VITE_API_BASE=https://<backend מ-FS-A>` (https → ה-wss נגזר אוטומטית). בלי זה → mode הדגמה (mock).
5. Deploy → לשלוח את הקישור לצוות.

> חלופה ב-CLI: `npm i -g vercel && vercel` (התחברות אינטראקטיבית — הריצו בעצמכם עם `! vercel`).
